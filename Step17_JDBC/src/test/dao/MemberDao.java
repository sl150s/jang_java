...
/*
 * Data Acces Object 만들어보기
 * - DB에 insert, update, delete  select 작업을 대신해 주는 객체를 생성할 클래스 설계하기
}
 */
public class MemberDao {

	// 회원 한 명의 정보를 저장하고 작업의 그 성공 여부를 리턴해주는 메소드=================================
	public boolean insert(MemberDto dto) {
		// 수정된 row의 갯수를 담을 지역변수를 미리 만들고 초기값 0 대입하기
		// 지역변수를 미리 만드는 이유 : try절에서도 지역변수 값을 주고, try절 바깥에서도 사용해야하기 때문에 미리 만들어둔다.
		int rowCount = 0;
		...
		try {
			...
			// executeUpdate : int 타입이 리턴된다.
			// 하나도 실행되지 않았으면 0이 리턴되고, 하나가 실행되면 1이 리턴, 10개가 수정되면 10이 리턴된다.
			// 때문에 0
		...
		// 변화된 rowCount 값을 확인해서 작업의 성공여부를 리턴해준다.
		if (rowCount > 0) {
			return true;
		} else {
			return false;
